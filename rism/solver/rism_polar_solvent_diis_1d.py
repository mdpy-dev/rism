...
# -*- encoding: utf-8 -*-
"""
file : rism_polar_solvent_diis_1d.py
created time : 2023/05/11
author : Zhenyu Wei
version : 1.0
...
class RISMPolarSolventDIIS1DSolver:
    def __init__(
        self,
        grid: FFTGrid,
        closure,
        ...
        """Create solver for a RISM equation with polar interaction in 1D spherical coordinate system using DIIS method
    ...
    def _rism_forward(self, gamma_s_matrix):
    ...
    def solve(
        ...
        for i in range(2):
            c_s_matrix = self._get_c_matrix(gamma_s_matrix)
            gamma_s_matrix_new, residual = self._rism_forward(gamma_s_matrix)
            gamma_s_list.append(gamma_s_matrix)
            residual_list.append(residual)
            gamma_s_matrix = gamma_s_matrix_new * 0.7 + gamma_s_matrix * 0.3
        ...
        while epoch < max_iterations and not is_finished:
            ...
            gamma_s_matrix = cp.zeros_like(gamma_s_matrix)
            for i in range(cur_subspace_size):
                gamma_s_matrix += factor[i] * gamma_s_list[i]
            gamma_s_matrix, _ = self._rism_forward(gamma_s_matrix)
            _, residual = self._rism_forward(gamma_s_matrix)
            gamma_s_list.append(gamma_s_matrix.copy())
            residual_list.append(residual.copy())
            epoch += 2

            gamma_s_list = gamma_s_list[-subspace_size:]
