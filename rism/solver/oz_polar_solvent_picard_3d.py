...
class OZPolarSolventPicard3DSolver:
    def __init__(
        self,
        grid: FFTGrid,
        closure,
    ...
    def _get_u(self, coordinate):
        ...
        return (ele_short).astype(CUPY_FLOAT), ele_long.astype(CUPY_FLOAT)
    ...
    def solve(
        ...
        exp_u_short = cp.exp(-self._beta * u_short).astype(CUPY_FLOAT)
        ...
        while epoch < max_iterations and not is_finished:
            ...
            c_k = c_short_k
            gamma_k = factor * c_k**2 / (1 - factor * c_k)
            gamma_pre = gamma.copy()
            gamma = cp.real(fft.ifftn(gamma_k))
            gamma = gamma * alta + gamma_pre * altb
            c_short, c_short_pre = self._closure(exp_u_short, gamma), c_short
