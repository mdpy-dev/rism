...
from rism.unit import UNIT_PRECISION
...
class Unit:
    ...
    def __repr__(self):
        return "<Unit object: %.2e %s at 0x%x>" % (
            self._relative_value,
            self._base_dimension.name,
            id(self),
        )

    def __str__(self):
        return "%.2e %s" % (self._relative_value, self._base_dimension.name)

    def __eq__(self, other) -> bool:
        err = np.abs(
            (self._relative_value - other.relative_value) / self._relative_value
        )
        if self._base_dimension == other.base_dimension and err < UNIT_PRECISION:
            return True
        else:
            return False
    ...
    def __add__(self, other):
        if isinstance(other, Unit):
            ...
            raise NotImplementedError(
                "+ between %s and mdpy.unit.Unit is not implemented" % (type(other))
            )
    ...
    def __radd__(self, other):
        if isinstance(other, Unit):
            ...
            raise NotImplementedError(
                "+ between mdpy.unit.Unit and %s is not implemented" % (type(other))
            )
    ...
    def __rtruediv__(self, other):
        if isinstance(other, Unit):
            ...
            return Unit(self._base_dimension**-1, 1 / self._relative_value)
        else:
            raise NotImplementedError(
                "/ between mdpy.unit.Unit and %s is not implemented" % (type(other))
            )
    ...
    def __pow__(self, value):
        try:
            ...
            return Unit(self._base_dimension**value, self._relative_value**value)
    ...
    def sqrt(self):
        ...
        Unit
            square root of ``self``
        """
        return Unit(self._base_dimension.sqrt(), np.sqrt(self._relative_value))
