...
from rism.unit import QUANTITY_PRECISION
...
class Quantity:
    ...
    def __eq__(self, other) -> bool:
        if isinstance(other, Quantity):
            if self._unit == other.unit:
                ...
                diff = self.value - (
                    other.unit.relative_value / self._unit.relative_value * other.value
                )
    ...
    def __lt__(self, other) -> bool:
        if isinstance(other, Quantity):
            if self._unit == other.unit:
                ...
                return (
                    self._value
                    < other.unit.relative_value
                    / self._unit.relative_value
                    * other.value
                )
            else:
                raise UnitDimensionMismatchedError(
            ...
            return NotImplementedError(
                "< between %s and mdpy.unit.Quantity is not implemented" % (type(other))
            )
    ...
    def __add__(self, other):
        if isinstance(other, Quantity):
            return Quantity(
                self._value
                + other.value * (other.unit.relative_value / self._unit.relative_value),
                self._unit
                + other.unit,  # Test wether the base dimension is same Or the dimension will be changed in the next step
            )
        else:
            return NotImplementedError(
                "+ between %s and mdpy.unit.Quantity is not implemented" % (type(other))
    ...
    def __rsub__(self, other):
        if isinstance(other, Quantity):
            return Quantity(
                other.value
                - self._value * (self._unit.relative_value / other.unit.relative_value),
                other.unit - self._unit,
            )
        else:
            return NotImplementedError(
                "- between mdpy.unit.Quantity and %s is not implemented" % (type(other))
            )
    ...
    def __mul__(self, other):
        if isinstance(other, Quantity):
            return Quantity(self._value * other.value, self._unit * other.unit)
        elif isinstance(other, Unit):
            return Quantity(self._value, self._unit * other)
        else:
            return NotImplementedError(
                "* between %s and mdpy.unit.Quantity is not implemented" % (type(other))
            )
    ...
    def __rtruediv__(self, other):
        if isinstance(other, Quantity):
            return Quantity(other.value / self._value, other.unit / self._unit)
        elif isinstance(other, Unit):
            return Quantity(1 / self._value, other / self._unit)
        else:
            return NotImplementedError(
                "-/between mdpy.unit.Quantity and %s is not implemented" % (type(other))
            )
    ...
    def __pow__(self, value):
        try:
            ...
            return Quantity(self._value**value, self._unit**value)
    ...
    def sqrt(self):
        ...
        Unit
            square root of ``self``
        """
        return Quantity(np.sqrt(self._value), self._unit.sqrt())

    def sum(self, *keys):
        return Quantity(np.sum(self._value, *keys), self.unit)
